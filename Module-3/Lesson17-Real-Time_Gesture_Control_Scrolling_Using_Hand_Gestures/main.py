...
def detect_gesture(landmarks, handedness):
    ...
    # Determine effective handedness from landmark positions (robust to palm/back)
    other_mcp_indices = [5, 9, 13, 17]  # Index, middle, ring, pinky MCP
    other_mcp_x = [landmarks.landmark[i].x for i in other_mcp_indices]
    avg_other_x = sum(other_mcp_x) / len(other_mcp_x)
    thumb_mcp_x = landmarks.landmark[mp_hands.HandLandmark.THUMB_MCP].x  # Landmark 2
    effective_handedness = "Right" if thumb_mcp_x > avg_other_x else "Left"
    
    # Check thumb using effective handedness
    thumb_tip = landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
    thumb_ip = landmarks.landmark[mp_hands.HandLandmark.THUMB_IP]
    if (effective_handedness == "Right" and thumb_tip.x > thumb_ip.x) or (effective_handedness == "Left" and thumb_tip.x < thumb_ip.x):
        fingers.append(1)
